...
//------------------------------------------------
CompressiveTracker::CompressiveTracker(std::shared_ptr<MyKalmanFilter::KalmanFilter> _kf)
    : kf(_kf)
{
	featureMinNumRect = 2;
	featureMaxNumRect = 4;	// number of rectangle from 2 to 4
    featureNum = 100;	// number of all weaker classifiers, i.e,feature pool
...
/* Description: Compute the coordinate of samples when detecting the object.*/
{
    ...
    {
        prediction = cv::Point(_objectBox.x, _objectBox.y);
    }

    int minrow = max(0,(int)prediction.y-(int)_srw);
    int maxrow = min((int)rowsz-1,(int)prediction.y+(int)_srw);
    int mincol = max(0,(int)prediction.x-(int)_srw);
    int maxcol = min((int)colsz-1,(int)prediction.x+(int)_srw);
	...
	for( r=minrow; r<=(int)maxrow; r++ )
		for( c=mincol; c<=(int)maxcol; c++ ){
            dist = (prediction.y-r)*(prediction.y-r) + (prediction.x-c)*(prediction.x-c);
...
void CompressiveTracker::processFrame(Mat& _frame, Rect& _objectBox)
{
    ...
    if(kf != NULL)
    {
        kf->correct(_objectBox.x, _objectBox.y);
    }
